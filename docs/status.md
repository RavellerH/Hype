...
| Daily Brief | ⚠️ | `daily_briefs` table ready, generated by `.github/workflows/daily-brief.yml` (Anthropic Claude Haiku). Needs `SUPABASE_URL`/`SUPABASE_KEY`/`ANTHROPIC_API_KEY` GitHub repo secrets set, and a GitHub PAT saved in the Brief tab to use "Generate Now" |
| Weekly Research | ⚠️ | `weekly_research` table ready, generated by `.github/workflows/weekly-research.yml`, same secrets/PAT requirement as Daily Brief |
...
| 🟡 4b | Save a GitHub PAT (`repo`+`workflow` scope) in Brief tab settings card | Unlocks: "Generate Now" / "Generate This Week" manual `workflow_dispatch` triggers |
