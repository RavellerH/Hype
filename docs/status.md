...
| Daily Brief | ⚠️ | `daily_briefs` table ready, generated by `.github/workflows/daily-brief.yml` (Anthropic Claude Haiku). Needs `SUPABASE_URL`/`SUPABASE_KEY`/`ANTHROPIC_API_KEY` GitHub repo secrets, and a `GH_PAT` Vercel env var (via `api/trigger-workflow.js` proxy) to use "Generate Now" |
| Weekly Research | ⚠️ | `weekly_research` table ready, generated by `.github/workflows/weekly-research.yml`, same secrets/`GH_PAT` requirement as Daily Brief |
...
| 🔴 2 | Add GitHub repo secrets `SUPABASE_URL`, `SUPABASE_KEY`, `ANTHROPIC_API_KEY` | Repo Settings → Secrets and variables → Actions — required for the Daily Brief / Weekly Research GitHub Actions cron to run at all |
