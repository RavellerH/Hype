...
async function fetchIndicatorsDirect() {
...
async function fetchIndicators() {
  ...
  try {
    ...
    raw = await r.json();
    _indData = {
      fear_greed: raw.fear_greed ? parseFG(raw.fear_greed) : null,
      bmsb: raw.btc_weekly ? parseBMSB(raw.btc_weekly) : null,
      pi_cycle: raw.btc_daily ? parsePiCycle(raw.btc_daily) : null,
      floors: raw.floors || { realized: 72000, balanced: 41000, cvdd: 45000, terminal: 290000 },
      fetched_at: Date.now(),
    };
  } catch(_) {
    _indData = await fetchIndicatorsDirect();
  }
  _indDataTs = Date.now();
...
async function loadIndicators() {
  ...
  const stripHtml = `<div class="ind-strip">
    ...
    <div class="ind-chip">
      ...
      <span class="${bmsb ? (bmsb.signal==='BULL'?'pos':bmsb.signal==='BEAR'?'neg':'yellow') : 'muted'} mono">${bmsb ? bmsb.signal : '—'}</span>
      ${bmsb ? `<span class="ind-badge ind-${bmsb.signal.toLowerCase()}">${bmsb.signal}</span>` : ''}
    </div>
    <div class="ind-chip">
      <span class="ind-label">Pi Cycle</span>
      <span class="${pi ? (pi.signal==='TOP'?'neg':pi.signal==='WARNING'?'yellow':'pos') : 'muted'} mono">${pi ? pi.proximity+'%' : '—'}</span>
