...
async function fetchNansenDirect() {
  ...
  const result = { netflows: data, holdings, fetched_at: Date.now() };
  _nansenCache = result;
...
async function fetchNansenData() {
  ...
  try {
    const resp = await fetch('/api/nansen/flow', {
      headers: { 'x-wallet': window.currentWallet || '' }
    });
    if (resp.ok) {
      const data = await resp.json();
      _nansenCache = data;
  ...
  (netflows || []).forEach(item => {
    ...
    if (prev && prev !== dir) {
      ...
      if (Notification.permission === 'granted') {
        new Notification('Hype — Flow Alert', { body: msg, icon: '/static/icons/icon-192.png' });
      }
    }
    _prevFlows[sym] = dir;
  });
...
function nansenKeyForm() {
  return `<div class="nansen-key-form">
    ...
    <div style="font-size:12px;color:var(--text-muted);margin-bottom:12px">Enter your Nansen API key to access Smart Money data. It will be saved locally and used as a direct fallback.</div>
...
function renderNansenTopMoversTab(netflows) {
  ...
  const filterChips = `<div style="display:flex;gap:6px;padding:10px 14px;border-bottom:1px solid var(--border)">
  ...
  try {
    ...
    if (!data) {
      ...
      content.innerHTML = `<div class="nansen-no-data">
        <div style="color:var(--red);font-weight:600;margin-bottom:8px">Unable to load Nansen data</div>
        <div style="font-size:12px;color:var(--text-muted);margin-bottom:12px">Backend unavailable and direct API call returned no data.</div>
